...
# getitem 게시글 데이터.
class LostItemArticle(Resource):
    def get(self, i_id):
        # 게시글 데이터.
        db = database.db_connect()
        sql = 'SELECT u.id as "author_id", u.name as "author_name", title, content, register_date, lost_date FROM lostitem AS gi JOIN user AS u ON gi.u_id = u.u_id WHERE gi.i_id={};'.format(i_id)
        curs = db.cursor()
        curs.execute(sql)
