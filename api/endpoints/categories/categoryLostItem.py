...
# category id를 통해 lostitem
class CategoryLostItem(Resource):

    def get(self, c_id):

        # db
        db = database.db_connect() #connection객체
        sql = 'select u.id, title from lostitem as gi join user as u on gi.u_id = u.u_id where c_id = \'{}\''.format(c_id)
